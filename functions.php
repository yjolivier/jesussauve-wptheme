...
function has_post_thumbnail_or_image(){
	global $post;
	$first_img = elogeb_get_first_image($post->ID);
...
function elogeb_get_first_image($post_id) {
...
function elogeb_modify_post_thumbnail_html($html, $post_id, $post_thumbnail_id, $size, $attr) {
	if( '' == $html ) {
		global $post, $posts;
		ob_start();
		ob_end_clean();
		
		$first_img = elogeb_get_first_image($post_id);
		if ( !empty( $first_img ) ){
...
add_filter('post_thumbnail_html', 'elogeb_modify_post_thumbnail_html', 10, 5);


/**
 * Display the first characters of the article content
 */
function elogeb_excerpt($max_char, $more_link_text = '...',$notagp = false, $stripteaser = 0, $more_file = '') {
...
function elogeb_admin_js($hook) {
  wp_enqueue_script( 'elogeb_admin_js', get_template_directory_uri() . '/js/admin.js' );
}
